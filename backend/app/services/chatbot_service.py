...
class ChatbotService:
    ...
    def get_chat_response(user_query, chat_history):
        ...
        for c in courses:
            ...
            chapter_list = ", ".join(chapter_names) if chapter_names else "Đang cập nhật"
        ...
        3. **Giới thiệu giá:** Khi giới thiệu giá của nhiều khóa học thì dùng cụm từ "giá chỉ từ {price} VNĐ" để tạo sự hấp dẫn. Còn nói về giá của 1 khóa cụ thể thì nói giá chính xác "Dạ, khóa học này có giá {price} VNĐ". Nếu khóa học miễn phí thì nói "Dạ, khóa học này hoàn toàn miễn phí ạ!".
        ...
        1. **Định dạng danh sách:** Khi hướng dẫn các bước, bắt buộc phải xuống dòng và đánh số thứ tự rõ ràng.
        2. **Hiển thị Thumbnail & Link (Mở tab mới & Kích thước nhỏ):**
           Mỗi khi gợi ý một khóa học, hãy sử dụng cấu trúc HTML dưới đây để thumbnail trông gọn gàng và tự động mở tab mới khi khách nhấn vào:
           
           <a href="URL_LINK" target="_blank">
             <img src="URL_ẢNH" width="160" alt="thumbnail" style="border-radius: 8px; margin-top: 5px;">
           </a>
           <br/>

           *(Lấy URL_ẢNH từ mục 'Thumbnail' và URL_LINK từ mục 'Link' trong dữ liệu).*
        """
